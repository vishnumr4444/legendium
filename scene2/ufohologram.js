...
/**
 * Creates the animated UFO hologram used in Scene 2.
 *
 * The hologram is a grouped collection of:
 *  - Concentric rotating ring shaders
 *  - A central "disc" shader
 *  - A neon cylinder column
 *  - Rising particle points and two additional shader planes.
 *
 * The returned object exposes:
 *  - `meshes`: an array containing the root `Group` to be attached to a UFO.
 *  - `update(delta)`: a function that must be called each frame to animate
 *    time‑based uniforms for all sub‑materials.
 *
 * Note: `scene`, `camera` and `renderer` are currently unused but are kept
 * for API symmetry with other FX helpers if future enhancements are needed.
 */
export function createHologram(scene, camera, renderer) {
  ...
  const ringMaterial1 = createRingMaterial(0x00aaff, 0.4);
  const plane1 = new THREE.Mesh(planeGeometry, ringMaterial1);
  plane1.rotation.x = Math.PI / 2;
  plane1.position.y = -1.0;

  const ringMaterial2 = createRingMaterial(0x00aaff, 0.54);
  ...
  let geometry = new THREE.CylinderGeometry(1.0, 1.0, 16, 32, 1, true);
  ...
  for (let i = 0; i < particleCount; i++) {
    let pos = randomPointInCylinder(1.0, 6);
    positions[i * 3] = pos.x;
  ...
  hologramGroup.scale.set(13, 20, 13);
